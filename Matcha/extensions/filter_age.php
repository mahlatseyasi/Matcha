    		...
    		<div class="container" style="position: relative; top: 1%; height: 80%; color: whitesmoke; z-index: 2; overflow-y: scroll;">
				...
				if (empty($_POST)){
					...
					if ($_POST['btnage'] === "18-25"){
						if ($_SESSION['auth']->orientation !== "M" && $_SESSION['auth']->orientation !== "F"){
							$req = $pdo->query("SELECT * FROM users WHERE age >= 18 AND age <= 25 AND reported = 0 
							ORDER BY ((lati-$ulati)*(lati-$ulati)) + ((longi - $ulongi)*(longi - $ulongi)), age ASC");	
						}
						else{
						...
						if ($_SESSION['auth']->orientation !== "M" && $_SESSION['auth']->orientation !== "F"){
							$req = $pdo->query("SELECT * FROM users WHERE age >= 25 AND age <= 35 AND reported = 0
							ORDER BY ((lati-$ulati)*(lati-$ulati)) + ((longi - $ulongi)*(longi - $ulongi)) ASC, age ASC");	
						}
						else{
						...
						if ($_SESSION['auth']->orientation !== "M" && $_SESSION['auth']->orientation !== "F"){
							$req = $pdo->query("SELECT * FROM users WHERE age >= 35 AND reported = 0 
							ORDER BY ((lati-$ulati)*(lati-$ulati)) + ((longi - $ulongi)*(longi - $ulongi)), age ASC");	
						}
						else{

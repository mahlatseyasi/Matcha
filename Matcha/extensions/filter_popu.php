    			...
    			<?php
    				...
    				else
    				{
						...
						if ($_POST['btnpopu'] === "0-100") {
							if ($_SESSION['auth']->orientation !== "M" && $_SESSION['auth']->orientation !== "F")
							{
								$req = $pdo->query("SELECT * FROM users WHERE popscore >= 0 AND popscore <= 100 AND reported = 0 
								ORDER BY ((lati-$ulati)*(lati-$ulati)) + ((longi - $ulongi)*(longi - $ulongi)), popscore ASC");	
							}
							else
							{
							...
							if ($_SESSION['auth']->orientation !== "M" && $_SESSION['auth']->orientation !== "F")
							{
								$req = $pdo->query("SELECT * FROM users WHERE popscore >= 100 AND popscore <= 200 AND reported = 0
								ORDER BY ((lati-$ulati)*(lati-$ulati)) + ((longi - $ulongi)*(longi - $ulongi)), popscore ASC");	
							}
							else
							{
							...
							if ($_SESSION['auth']->orientation !== "M" && $_SESSION['auth']->orientation !== "F")
							{
								$req = $pdo->query("SELECT * FROM users WHERE popscore >= 200 AND reported = 0 
								ORDER BY ((lati-$ulati)*(lati-$ulati)) + ((longi - $ulongi)*(longi - $ulongi)), popscore ASC");	
							}
							else
							{
